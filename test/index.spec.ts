...
describe("Encryption", () => {
  it("Should pass standard test suite", () => {
    const data = tests2[0];
    const compute = unixcrypt.encrypt(data[1], data[0]);
    expect(compute).to.equal(data[2]);
    expect(unixcrypt.verify(data[1], data[2])).to.be.true;
  });
  ...
  it("Should throw an exception when used with any other crypto than sha256 or sha512", () => {
    ...
    expect(() => unixcrypt.encrypt(data[1], data[0])).to.throw("Only sha256 and sha512 is supported by this library");
    expect(() => unixcrypt.verify(data[1], data[2])).to.throw("Only sha256 and sha512 is supported by this library");
  });
  ...
  it("Should throw an exception when salt contains invalid characters", () => {
    ...
    expect(() => unixcrypt.encrypt(data[1], data[0])).to.throw("Invalid salt string");
    expect(() => unixcrypt.verify(data[1], data[2])).to.throw("Invalid salt string");
  });
  ...
  it("Should be possible to only specify the SHA-type", () => {
    ...
    const compute = unixcrypt.encrypt(plaintext, salt);
    expect(unixcrypt.verify(plaintext, compute)).to.be.true;
  });
  ...
  it("Should be possible to not specify a salt at all", () => {
    const plaintext = "Plaintext password";
    // this should not throw an exception
    const compute = unixcrypt.encrypt(plaintext);
    expect(unixcrypt.verify(plaintext, compute)).to.be.true;
  });
  ...
  //   expect(unixcrypt.verify(plaintext, hash)).to.be.true;
  // });
